...
public class ConsoleActivity extends AppCompatActivity
    ...
    //speed tracking and stored coefficient
    private float speed = 0, multiplier = 1f;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        multiplier = application.getSpeedCalibration();
    }
    ...
    public void onLocationChanged(Location location) {
        ...
        if (!consoleData.isInSearch()){
            ...
            animateSpeed(location.getSpeed() * this.multiplier);
        }
    ...
    private void init() {
        ...
        //if walk through hasn't occurred, we start a walk through
            ...
            Statics.showConsoleShowcase(this, new Statics.OnShowcaseGoneListener() {
                ...
                public void onShowcaseViewDidHide(ShowcaseView showcaseView) {
                    ...
                    if (Statics.isLocationEnabled(ConsoleActivity.this)) {
                        locationProvider.startUpdates();
                        animateSpeed(80f);
                        new Handler().postDelayed(() -> animateSpeed(0), 600);
                    }
                }
            });
    ...
    private void showCalibrator() {
        AlertDialog calibrator = new AlertDialog.Builder(this)
                .setTitle(R.string.speed_calibration_title)
                .setIcon(R.mipmap.ic_launcher_round)
                .setPositiveButton(android.R.string.ok, (dialog, which) -> {
                    EditText editText = ((AlertDialog) dialog).findViewById(R.id.calibration);
                    if (editText != null) {
                        try {
                            this.multiplier = application.setSpeedCalibration(Float.parseFloat(editText.getText().toString()));
                            Toast.makeText(ConsoleActivity.this, "Speed calibration successful", Toast.LENGTH_SHORT).show();
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    }
                ...
                .create();
        ConstraintLayout layout = (ConstraintLayout) LayoutInflater.from(this).inflate(R.layout.dialog_gps_callibration, findViewById(R.id.root), false);
        calibrator.setView(layout);
        EditText editText = layout.findViewById(R.id.calibration);
        if (editText != null) editText.setText(String.valueOf(application.getSpeedCalibration()));
        calibrator.show();
    }

    private void animateSpeed(float newSpeed){
        ValueAnimator animator = ValueAnimator.ofFloat(speed, newSpeed);
        speed = newSpeed;
        animator.setDuration(400);
        animator.addUpdateListener(animation -> speedometer.setText(String.valueOf(((Float)animation.getAnimatedValue()).intValue())));
        animator.start();
    }
}